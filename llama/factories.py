...
from llama.entity_extractor import parse_llm_output_to_enhanced_triplets
...
class ModelFactory:
    ...
    def create_embedding_model():
        """创建 Embedding 模型 - 仅使用在线 API"""
        modules = LlamaModuleFactory.get_modules()
        if not modules:
            return None

        try:
            from llama.custom_siliconflow_embedding import CustomSiliconFlowEmbedding

            logger.info("使用在线嵌入模型 via SiliconFlow")
    ...
    def create_lightweight_llm():
        ...
        try:
            ...
            lightweight_model = (
                API_CONFIG["siliconflow"].get("lightweight_model")
    ...
    def create_graph_store():
        ...
        try:
            ...
            if not username or not password or not url or not database:
                logger.warning(
                    "Neo4j 配置不完整，回退到内存图存储 SimplePropertyGraphStore"
                )
...
class ExtractorFactory:
    ...
    def create_extractor(
        llm, extract_prompt: str = None, max_triplets_per_chunk: int = None
    ):
        """创建实体提取器 - 使用 DynamicLLMPathExtractor"""
        modules = LlamaModuleFactory.get_modules()
        if not modules:
            return None
        ...
        try:
            ...
            max_triplets = (
                max_triplets_per_chunk or EXTRACTOR_CONFIG["max_triplets_per_chunk"]
            )

            return modules["DynamicLLMPathExtractor"](
                llm=llm,
                extract_prompt=prompt,
                parse_fn=parse_llm_output_to_enhanced_triplets,
